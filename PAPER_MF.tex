...
%	In this study, reinitialization is not used at the end of each time step, unlike the low-order level set methods \cite{sussman_level_1994,nagrath_computation_2005,smolianski_finite_2005}, which reduces the computational cost. (\textcolor{red}{TODO}: Peki ne yapıldı bunun yerine?)
	...
	%To control aliasing errors, volume and surface integrals are computed using cubature rules. For triangles, area integration is performed using symmetric cubature rules \cite{ronald_monomial_1999}  and edge integrals are computed using Gauss quadrature rules. Tetrahedral elements use  non-symmetric cubature rules for volume integration utilizing tensor product of Gauss-Lobatto-Legendre and Gauss-Radau-Jacobi distributions and symmetric cubature rules  for area integration. Volume and surface integrals  also represents the area and edge integration for 2D problems. 
